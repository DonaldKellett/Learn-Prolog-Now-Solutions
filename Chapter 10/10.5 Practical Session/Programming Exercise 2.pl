% Utilities from 10.5 Practical Session Programming Exercise 1 Part 3
...
helper([], _, A, A).
helper([H | T], Term, Acc, Result) :- nu(H, Term), !, helper(T, Term, Acc, Result).
helper([H | T], Term, Acc, Result) :- helper(T, Term, [H | Acc], Result).

unifiable(List1, Term, List2) :- helper(List1, Term, [], List2Rev), reverse(List2Rev, List2).
